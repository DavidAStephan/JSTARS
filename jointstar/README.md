...
**Final-spec estimation (one call, ~48 min on 6 cores):**
```matlab
results = jointstar.estimate('data.csv', 'NParticles', 2000, ...
    'Seed', 42, 'Horseshoe', true, 'HierKappa', true);
```
Final results: `results/cp7b/`. Checkpoint history + owner rulings: `checkpoints/`.
Spec-decision switches live as labelled lines in `horseshoePriors.m`
(EXCLUDE_GAP) and `defaultPriors.m` (sign restrictions, calibrated 30bp).
...
docs/, data/, results/  populated in later checkpoints
...
```matlab
cd jointstar; startup            % or startup(8) to open a parpool
