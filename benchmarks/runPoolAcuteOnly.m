function runPoolAcuteOnly(dataFile, outRoot, seeds)
%RUNPOOLACUTEONLY Seed sweep for the 'PoolAcuteOnly' kappa-prior flag.
%
%   runPoolAcuteOnly(dataFile, outRoot, seeds)
%
%   Runs the 'PoolAcuteOnly' option ON TOP OF the full baseline trio +
...
%   runNeffSweep.m). This is the DRIVER only -- it is not invoked by
%   this commit; the orchestrator runs it explicitly when ready.
%
%   Arguments (all optional):
%       dataFile  -- input CSV (default: 'data.csv')
%       outRoot   -- output root directory (default:
%                    fullfile('results','experiments','PoolAcuteOnly'))
...
if nargin < 2 || isempty(outRoot)
    outRoot = fullfile('results', 'experiments', 'PoolAcuteOnly');
end
...
for k = 1:nSeeds
    ...
    if isfile(fullfile(od, 'posterior_summary.csv'))
        fprintf('runPoolAcuteOnly: seed %d already done (%s); skipping.\n', s, od);
        nSkipped = nSkipped + 1;
        continue
    end
    ...
    % Timestamped start
    tStart = datetime('now');
    fprintf('[%s] runPoolAcuteOnly: starting seed %d -> %s\n', ...
        tStart, s, od);
    tic;
    ...
    try
        ...
        tEnd = datetime('now');
        fprintf('[%s] runPoolAcuteOnly: seed %d completed (%.2f minutes)\n', ...
            tEnd, s, elapsedMin);
        ...
        elapsed = toc;
        elapsedMin = elapsed / 60;
        fprintf('runPoolAcuteOnly: seed %d FAILED after %.2f minutes\n', s, elapsedMin);
...
% ---- final summary ------------------------------------------------------
fprintf('\n');
fprintf('runPoolAcuteOnly summary:\n');
fprintf('  Completed: %d\n', nCompleted);
fprintf('  Skipped:   %d\n', nSkipped);
fprintf('  Failed:    %d\n', nFailed);
if nFailed > 0
